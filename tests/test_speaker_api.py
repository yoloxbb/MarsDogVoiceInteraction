...
def test_upload_appends_audio_and_embedding_under_normalized_name(
    isolated_speaker_storage: Path,
) -> None:
    manager = SpeakerEnrollmentManager()
    manager.set_speaker_extractor(_Extractor())

    first = manager.enroll_speaker_from_audio(" 张 三 ", _wav(), vad=_vad())
    second = manager.enroll_speaker_from_audio("张 三", _wav(), vad=_vad())

    directory = isolated_speaker_storage / "speakers" / "张_三"
    ...
    assert SpeakerEnrollmentManager.list_enrolled_speakers() == ["张_三"]

    provider = _SpeakerProvider()
    assert manager.sync_to_provider(provider) == 1
    assert provider._manager.values["张_三"] != [0.0, 0.0]


def test_storage_allows_at_most_five_people_and_crud_releases_capacity(
    isolated_speaker_storage: Path,
) -> None:
    manager = SpeakerEnrollmentManager()
    manager.set_speaker_extractor(_Extractor())
    ...
    assert rejected["code"] == "speaker_limit_reached"
    assert not (isolated_speaker_storage / "speakers" / "person-6").exists()
    assert existing["ok"] is True
    assert existing["shots"] == 2

    renamed = manager.rename_speaker("person-1", "owner")
    conflict = manager.rename_speaker("owner", "person-2")
    assert renamed["ok"] is True
    assert conflict["status"] == 409
    assert (isolated_speaker_storage / "speakers" / "owner").exists()
    assert not (isolated_speaker_storage / "speakers" / "person-1").exists()

    assert manager.delete_speaker("person-5")["ok"] is True
...
def test_fastapi_accepts_multipart_name_and_wav() -> None:
    calls: list[tuple[str, bytes]] = []

    def handler(name: str, payload: bytes) -> dict[str, object]:
        calls.append((name, payload))
        return {"ok": True, "name": "张_三", "shots": 1}
    ...
    assert response.json()["name"] == "张_三"
    assert len(response.json()["request_id"]) == 32
    assert calls == [("张 三", _wav())]
...
def test_fastapi_returns_422_for_corrupt_or_speechless_wav(
    ...
    speechless = _post(speechless_server, name="Silent")
...
def test_fastapi_maps_five_person_limit_to_http_409() -> None:
    ...
    response = _post(server, name="person-6")
...
def test_fastapi_lists_renames_and_deletes_speakers() -> None:
    speakers = {"Alice": {"name": "Alice", "shots": 1}}
    ...
    rejected_path = _request(
        ...
        json={"name": "Owner", "storage_root": "/tmp/forbidden"},
    )
    deleted = _request(server, "DELETE", "/api/v1/speakers/Owner")
    missing = _request(server, "DELETE", "/api/v1/speakers/Owner")

    assert listed.status_code == 200
    assert listed.json()["max_speakers"] == 5
    assert listed.json()["max_samples_per_speaker"] == 5
    assert renamed.status_code == 200
    assert renamed.json()["name"] == "Owner"
