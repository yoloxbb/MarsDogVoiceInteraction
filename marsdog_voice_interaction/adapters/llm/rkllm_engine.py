...
class RKLLMEngine:
    ...
    def _disable_chat_template(self) -> None:
        """通过传递空字符串禁用自动聊天模板，保证 prompt 原样传递。"""
        if self._rkllm_set_chat_template is None:
            if self.verbose:
                print("[RKLLM] rkllm_set_chat_template not available, skip disable")
            return
        ...
        if self.verbose:
            print(f"[RKLLM] disabled chat template, ret={ret}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RKLLMEngine single prompt test")
    parser.add_argument("--model", "--rkllm_model_path", dest="model_path", required=True)
    parser.add_argument("--lib_path", default=None)
    parser.add_argument("--platform", default="rk3588",
                        choices=["rk3588", "rk3576", "rv1126b", "rk3562"])
    parser.add_argument("--prompt", default="你好，请用中文简单介绍一下你自己。")
...
"""
   ...
   uv run python -m marsdog_voice_interaction.adapters.llm.rkllm_engine \
     --model /home/cat/xbb/models/llm/qwen2_5_0_5b_rk3588_w8a8.rkllm \
